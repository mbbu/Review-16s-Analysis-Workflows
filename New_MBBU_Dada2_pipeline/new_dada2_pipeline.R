...
taxa <- assignTaxonomy(seqtab.nochim,"silva_nr99_v138.1_train_set.fa",tryRC = TRUE)

taxa <- addSpecies(taxa, "silva_species_assignment_v138.1.fa.gz",tryRC = TRUE)
...
samdata <- read.table("metadata.tsv", header = TRUE, sep="\t", row.names = samples_F)
samples_F
samdata
